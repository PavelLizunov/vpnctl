...
- **MINOR — amneziawg H1-H4 / Jc obfs params not randomised per-server**:
  `render_config` falls back to literal defaults (H1=1..H4=4) → all
  amneziawg nodes share magic constants → DPI-fingerprintable. (amneziawg
  not deployed in prod today.) Mint `amneziawg.{jc,jmin,jmax,s1,s2,h1..h4}`
  per server in the secret bootstrap.
