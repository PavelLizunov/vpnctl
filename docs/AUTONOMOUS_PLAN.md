...
| 10 | open · hotfix | **I4 follow-up — set `VPNCTLD_TRUSTED_PROXIES=192.168.0.207` on prod env** | Regression introduced by Bundle 1 `f881ba9` (2026-05-22, audit fix I4 — empty-by-default trusted-proxies). On prod-deploy 192.168.0.236 the env var was NOT set, so 192.168.0.207 (nginx) is now `untrusted` → every `/sub` fetch through nginx gets logged with `peer = 192.168.0.207` (LAN RFC1918) → fires `sub_access.suspicious_local_ip:<user>` warning **for every legit client through nginx** (10+ false-positive alerts accumulated within hours). **Fix:** append `Environment=VPNCTLD_TRUSTED_PROXIES=192.168.0.207` to `/etc/systemd/system/vpnctld.service.d/override.conf` (or to whatever env-file the unit reads — check via `systemctl cat vpnctld`), `daemon-reload + restart vpnctld`. Then ack-all current backlog via the /admin/alerts bulk-ack button. Post-fix verify: next `/sub` from nginx logs the real upstream client IP in `sub_access_log.ip`, NOT 192.168.0.207. **Document the regression** in CLAUDE.md «Грабли» so future operators provisioning a second daemon instance don't repeat it. | ~10 (config + doc) |
