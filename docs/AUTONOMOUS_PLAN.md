...
| 10 | [shipped 2026-05-23] | **I4 follow-up — env-var on prod + defensive detector gate** | Done in two parts: (a) `VPNCTLD_TRUSTED_PROXIES=192.168.0.207` appended to `/etc/vpnctl/vpnctld.env` on 192.168.0.236, `vpnctld` restarted, 10 false-positive `sub_access.suspicious_local_ip` alerts bulk-acked via `POST /admin/alerts/ack-all`. (b) `access_log.rs::is_trusted_reverse_proxy` gate added — even if a future operator forgets the env-var, the LAN-fetch detector now suppresses alerts when the peer IP is in `trusted_proxies()`. Documented in CLAUDE.md «Грабли» so the regression doesn't repeat on the next reverse-proxy'd daemon instance. | ~50 (config + code + doc) |
| 11 | [shipped b4608d2] | **Alert auto-recovery — `user.traffic_limit` + `server.fingerprint.drift`** | Both `check_*` health-monitor passes now call `inv.ack_open_alerts(&kind, server_id)` on the early-exit «condition is fine» branch. Silent ack (no `*.recovered` info alert) — matches existing `ack_open_alerts` doc-comment policy. Removes the «manually-ack-30-stale-warnings every month» tax. 1 test pinning the two-tick fire→recover round-trip. | ~50 |
| 12 | [shipped 07a9fd3] | **Disabled-user count on dashboard Users tile (B1.user surface)** | `count_disabled_users()` inventory helper + `DashboardStats.disabled_users` field + amber «N paused» sub-line in the Users tile (rendered ONLY when count > 0 — quiet contract). Direct link to /admin/users for drill-in. 2 smoke tests for zero-and-nonzero contracts. | ~70 |
| 13 | [shipped 5e7be3f] | **A3 per-server 24h resource-trend sparklines** | New `server_detail_resource_trend_section(rows, lang)` on /admin/servers/<id> — three SVG sparklines (disk %, mem-used %, sing-box log MiB) via the existing reusable `sparkline_svg` helper. Rendered between the «right-now» hero and the live-activity tile so the page flow is: snapshot → SLO over time → resource trend over time → live activity. Live demo on `de` showed predictable log-file climb (157 MiB / climbing → 500 MiB alert ETA visible to the eye). | ~180 |
| 14 | [shipped 9620e9b] | **B2 bulk grant / revoke on server-detail** | `POST /admin/servers/{id}/grants/_grant-all` (no confirm — idempotent, reversible per-user) + `.../_revoke-all` (double-submit confirm matching the C-3.4 delete-user shape — operator retypes the server id). Single summary audit row per batch (`server.grants.bulk_grant` / `bulk_revoke`), not N per-user rows. Buttons render only when meaningful (grant-all hidden when 0 ungranted; revoke-all hidden when 0 granted). | ~420 |
| 15 | [shipped 85aa251] | **A5 fleet-wide search /admin/search?q=** | Substring match across users (id/uuid/sub_token/device_id), servers (id/address), alerts (kind/summary). Three `search_*` inventory helpers, single handler runs them in parallel via `tokio::try_join`, results page groups hits per type with click-through to canonical detail pages. Compact `<form method="get">` search bar in the nav, right side. Audit search stays at /admin/audit (its own filter form is the right surface for that data shape). Bookmarkable URLs. | ~430 |
